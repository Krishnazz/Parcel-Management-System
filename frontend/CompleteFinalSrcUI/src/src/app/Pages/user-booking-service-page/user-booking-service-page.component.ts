...
import { UserBookingServiceService } from '../services/user-booking-service.service';

@Component({
  selector: 'app-user-booking-service-page',
  standalone: true,
  imports: [UserHeaderComponent, ReactiveFormsModule, CommonModule, FormsModule],
  templateUrl: './user-booking-service-page.component.html',
  styleUrl: './user-booking-service-page.component.css'
})
export class UserBookingServicePageComponent {
...
submitBooking() {
  ...
  this.bookingService.bookParcel(this.userId, parcel).subscribe({
    next: (bookingId) => {
      this.bookingId = bookingId;
      this.currentStep = 5;
      // Fetch invoice
      this.bookingService.getInvoice(bookingId, this.userId).subscribe({
        next: (invoice) => {
  ...
  senderInfo = {
    name: this.obj.userId,
    address: this.obj.address,
    contact: this.obj.mobileNumber
  };
  receiverInfo = {
    name: '',
    address: '',
  ...
  validateStep(step: number): boolean {
    ...
    if (step === 1) {
      if (!this.senderInfo.name) isValid = false;
      if (!this.senderInfo.address) isValid = false;
      if (!this.senderInfo.contact) isValid = false;
